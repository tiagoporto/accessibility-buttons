  ...
  if (config.handlebars) {
    return gulp
      ...
      .pipe(
        handlebars({
          partials:
            paths.html.src + basePaths.handlebars.partials.src + '**/*.hbs',
        })
      )
...
gulp.task('styles', function () {
  return gulp
    .src([paths.styles.src + '*.styl', '!' + paths.styles.src + '_*.styl'])
...
gulp.task('bitmap-sprite', function () {
  ...
  sprite.css
    .pipe(gulp.dest(paths.styles.src + 'helpers'))
    .pipe(
      plugins.notify({ message: 'Bitmap sprite task complete', onLast: true })
    )
...
gulp.task('vetor-sprite', function () {
  ...
  spriteOptions.mode.css.render[extensionStyle].dest =
    '../../' + paths.styles.src + 'helpers/_vetor-sprite.' + extensionStyle
...
gulp.task('images', function () {
  ...
  var svg = gulp
    .src([paths.images.src + '**/*.svg', '!' + paths.sprite.src + '**/*'])
    .pipe(plugins.plumber())
    .pipe(plugins.newer(paths.images.dest))
    .pipe(plugins.svg2png())
    .pipe(gulp.dest(paths.images.dest))
...
gulp.task('scripts', function () {
  var babelOption = { presets: ['@babel/env'] }
...
gulp.task('serve', function () {
  ...
  gulp.watch(paths.sprite.src + '**/*.{png,gif}', [
    'bitmap-sprite',
    browserSync.reload,
  ])

  gulp.watch(paths.sprite.src + '**/*.svg', [
    'vetor-sprite',
    'styles',
    browserSync.reload,
  ])

  gulp.watch(paths.images.dest + '**/*.svg', [
    'svg2png',
    'handlebars',
    browserSync.reload,
  ])
  ...
  gulp.watch(paths.scripts.src + 'settings/**/*.js', [
    'vendor-scripts',
    browserSync.reload,
  ])
  ...
  gulp.watch(basePaths.dest + '**/*.{html,php,json}', [
    'svg-inline',
    browserSync.reload,
  ])
})
